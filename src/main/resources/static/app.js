...
function appendActionCell(row, request) { const cell = document.createElement("td"); if (currentUserRole === "ADMIN" && request.status === "OPEN") { cell.append(actionButton("Editar", () => editPurchaseRequest(request.id)), actionButton("Cancelar", () => cancelPurchaseRequest(request.id))); } if (currentUserRole === "ADMIN" && request.status === "CANCELLED") cell.append(actionButton("Excluir", () => deletePurchaseRequest(request.id))); row.append(cell); }
function actionButton(label, action) { const button = document.createElement("button"); button.className = "table-action"; button.type = "button"; button.textContent = label; button.addEventListener("click", action); return button; }
...
async function cancelPurchaseRequest(id) { if (!window.confirm("Cancelar esta requisição? O histórico será mantido.")) return; await performRequest(`${apiUrl}/${id}/cancel`, "POST"); }
async function deletePurchaseRequest(id) { if (!window.confirm("Excluir definitivamente esta requisição cancelada? Esta ação não pode ser desfeita.")) return; await performRequest(`${apiUrl}/${id}`, "DELETE"); }
async function performRequest(url, method) { const response = await fetch(url, { method, headers: { [csrfToken.headerName]: csrfToken.token } }); if (!response.ok) { window.alert(await readErrorMessage(response)); return; } await loadPurchaseRequests(); }
