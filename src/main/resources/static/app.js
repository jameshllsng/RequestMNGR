...
async function loadAuthenticatedUser() { const response = await fetch("/api/auth/me"); if (response.status === 401) { window.location.assign("/login.html"); return false; } if (!response.ok) throw new Error("Não foi possível identificar o usuário autenticado."); const user = await response.json(); document.querySelector("#current-user").textContent = user.username; return true; }
